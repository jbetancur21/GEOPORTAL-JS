...
if (id === "0") {
    ...
    // URL de tu API
    const apiUrl = 'http://127.0.0.1:8000/mapas/' + id;
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            $("#capasCalculos").change(function () {
                ...
                fetch('http://localhost:8080/geoserver/geoportal/ows?service=WFS&version=1.0.0&request=GetFeature&typeNames=geoportal%3A' + capaGrafico.trim() + '&outputFormat=application%2Fjson')
                    .then(response => response.json())
                    .then(data => {
                    ...
                    $("#graficos").change(function () {
                        ...
                        if (usoGrafico != "Gráficos...") {
                            ...
                            function fetchData(elementId) {
                                ...
                                return fetch('http://localhost:8080/geoserver/geoportal/ows?service=WFS&version=1.0.0&request=GetFeature&typeNames=geoportal%3A' + capaGrafico.trim() + '&outputFormat=application%2Fjson')
                                    .then(response => response.json())
                                    .then(data => {
